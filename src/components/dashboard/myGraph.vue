...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        ...
        if (this.data[0].length === 2) {
          ...
          this.data.forEach(item => {
            ...
            edges.push({
              source: objects.indexOf(object1),
      ...
      append(svgEdges, svgNodes, edges, nodes, svg, force, drag) {
        // 为节点和边添加具体的形状以及绑定相关函数
        var self = this
        // 连线聚焦边框
        svgEdges.append('path')
          ...
          .on('mouseover', function (d, i) {
            ...
            // 在header中显示关系名字
            if (d.rela === 'seeAlso') {
              self.header = 'Relation: ' + d.rela + ' ( SeeAlso in ' + d.target.name + ' )'
            } else {
              self.header = 'Relation: ' + d.rela + ' ( ' + d.target.name +
          ...
          .on('mouseout', function (d, i) {
            ...
            // 在header中显示查询信息
            self.header = 'Displaying ' + nodes.length + ' nodes, ' + edges.length + ' relationships.'
          })
        ...
        svgNodes.append('circle')
          .attr({
            ...
            'r': function (d) {
              ...
              var radius = '34'
              if (self.timesMax !== 0) {
                radius = (34 + (d.times * 20 / self.timesMax)).toString()
              }
              return radius + 'px'
            }
          })
        ...
        var nodesOutline = svgNodes.append('circle')
          .attr({
            ...
            'r': function (d) {
              ...
              var radius = '30'
              if (self.timesMax !== 0) {
                radius = (30 + (d.times * 20 / self.timesMax)).toString()
              }
              return radius + 'px'
            }
          })
          .style({
            'fill': function (node) {
              ...
              if (node.name === 'Tencent') {
                color = '#4dbcfa'
              } else if(node.name === 'LCLT') {
                color = '#bad7df'
              } else if(node.name === 'GST') {
                color = '#ffe2e2'
              } else if(node.name === 'Keke') {
                color = '#ffd2a5'
              } else if(node.name === 'Lulu') {
                color = '#99ddcc'
              } else {
                color = '#ef89f9'
              }
          ...
          .on('mouseup', function (d, i) {
            ...
            if (!hasMove) {
              if (select === false) {
                ...
                var linear = 0
                if (self.timesMax !== 0) {
                  linear = d.times * 20 / self.timesMax
                }
                // 功能模块图标的额外位移
                ...
                funcGroup.append('path')
                  ...
                  .on('mouseover', function (d, i) {
                    ...
                    self.header = 'Remove node from the visualization'
                  })
                  .on('mouseout', function (d, i) {
                    d3.select(this)
                      .style({
                ...
                funcGroup.append('path')
                  ...
                  .on('mouseover', function (d, i) {
                    ...
                    self.header = 'Expand child relationships'
                  })
                  .on('mouseout', function (d, i) {
                    d3.select(this)
                      .style({
                  ...
                  .on('click', function (d, i) {
                    ...
                    select = false
                    var newData
                    axios.post('http://172.20.10.6:8888/n_step_object/', {
                      limit: 25,
                      start_node: d.name,
                      relationship: '',
                      step_number: 1,
                      ...
                      if (newData.length === 0) {
                        ...
                        newData.forEach(item => {
                          // newData中的数据一定以该节点为起点
                          if (nodesName.indexOf(item[2][0]) === -1) {
                            nodes.push({
                              name: item[2][0],
                              times: item[2][1]
                            })
                            if (item[2][1] > self.timesMax) {
                              // 如果新节点的times比timesMax更大则更新timeMax
                              self.timesMax = item[2][1]
                            }
                            nodesName.push(item[2][0])
                          }
                          edges.push({
                            source: nodesName.indexOf(item[0][0]),
                            target: nodesName.indexOf(item[2][0]),
                            rela: item[1][0]
                          })
                        })

                        svgEdges = svgEdges.data(force.links()).enter()
                          .append('g')
                          .attr('class', 'relationship')

                        svgNodes = svgNodes.data(force.nodes()).enter()
                          .append('g')
                          .attr('class', 'node')
                          .call(force.drag) // 使得节点能够拖动

                        self.append(svgEdges, svgNodes, edges, nodes, svg, force, drag)
                ...
                funcGroup.append('path')
                  ...
                  .on('mouseover', function (d, i) {
                    ...
                    self.header = 'Show the relationships to another node'
                  })
                  .on('mouseout', function (d, i) {
                    d3.select(this)
                      .style({
                ...
                funcGroup.append('path')
                  ...
                  .on('mouseover', function (d, i) {
                    ...
                    self.header = 'Unlock the node to re-layout the node'
                  })
                  .on('mouseout', function (d, i) {
                    d3.select(this)
                      .style({
                ...
                funcGroup.append('path')
                  .attr({
                    ...
                    'transform': 'translate(' + (29 + linearIcon).toString() + ', ' + (-(44 + linearIcon)).toString() +
                      ') scale(0.02)',
                    'pointer-events': 'none',
                    'class': 'remove'
                  })
                ...
                funcGroup.append('path')
                  .attr({
                    ...
                    'transform': 'translate(' + (29 + linearIcon).toString() + ', ' + (29 + linearIcon).toString() +
                      ') scale(0.016)',
                    'pointer-events': 'none',
                    'class': 'expand'
                  })
                ...
                funcGroup.append('path')
                  .attr({
                    ...
                    'transform': 'translate(' + (-(55 + linearIcon)).toString() + ', ' + (20 + linearIcon).toString() +
                      ') scale(0.035)',
                    'pointer-events': 'none',
                    'class': 'link'
                  })
                ...
                funcGroup.append('path')
                  .attr({
                    ...
                    'transform': 'translate(' + (-(48 + linearIcon)).toString() + ', ' + (-(43 + linearIcon)).toString() +
                      ') scale(0.018)',
                    'pointer-events': 'none',
                    'class': 'unlock'
                  })
      ...
      loadGraph() {
        ...
        this.header = 'Displaying ' + nodes.length + ' nodes, ' + edges.length + ' relationships.'
        ...
        force.on('tick', function () { // 对于每一个时间间隔
          // 更新连线坐标系
          d3.selectAll('.relationship').attr({
          ...
          // 更新连线
          d3.selectAll('.edgeOutline').attr({
            'd': function (d) {
              var sourceRad = 31
              var targetRad = 31
              // if (self.timesMax !== 0) {
              //   sourceRad = 31 + (d.source.times * 20 / self.timesMax)
              //   targetRad = 31 + (d.target.times * 20 / self.timesMax)
              // }
              var len = Math.sqrt((Math.pow((d.source.x - d.target.x), 2) + Math.pow((d.source.y - d.target.y),
                2))) - sourceRad - targetRad - d.rela.length * 5
              // 画线+箭头形状
              return 'M' + sourceRad + ' 0.5' +
                ' h ' + (len / 2).toString() +
                ' v ' + (-1).toString() +
                ' h ' + (-(len / 2)).toString() + ' Z ' +
                ' M ' + (targetRad + len / 2 + d.rela.length * 5).toString() + ' ' + 0.5 +
                ' h ' + (len / 2 - 7).toString() +
                ' v ' + 3 +
                ' l ' + 7 + ' ' + (-3.5).toString() +
          ...
          // 更新连线文字位置
          d3.selectAll('.edgeTexts').attr({
            'x': function (d) {
              return (Math.sqrt((Math.pow((d.source.x - d.target.x), 2) + Math.pow((d.source.y - d.target.y),
                2)))) / 2
            },
            'y': '3px'
          })
          // 更新连线遮罩坐标
          d3.selectAll('.overlay').attr({
            'd': function (d) {
              var len = Math.sqrt((Math.pow((d.source.x - d.target.x), 2) + Math.pow((d.source.y - d.target.y),
                2)))
